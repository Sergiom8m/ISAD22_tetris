...
class Identifikazioa(object):
    ...
    def __init__(self):
        ...
        self.erabiltzaile = tk.Entry(self.window, bg=botoi_kolor, justify=tk.LEFT, width=23, textvariable="Erabiltzailea",
        ...
        self.pasahitza = tk.Entry(self.window, bg=botoi_kolor, justify=tk.LEFT, width=23, textvariable="Pasahitza", font=("Times New Roman", 16))
        self.pasahitza.place(x=70, y=200)

        buttonb = tk.Button(self.window, bg=botoi_kolor, text="Pasahitza berreskuratu", font=("Times New Roman", 16), command=self.identifik_berresk)
        ...
        buttonerr = tk.Button(self.window,bg=botoi_kolor, text="Sartu", width=8, font=("Times New Roman", 16), command=self.identifik_erregis)
    ...
    def identifik_erregis(self):
        ...
        if ((len(id) != 0) & (len(p) != 0)):
            ...
            ezDago = a is None
            if (ezDago):
                error = tk.Label(self.window,bg=atzeko_kolor, fg="red",  text='Ez dago erabiltzaile hori                  ', font=("Times New Roman", 16))
